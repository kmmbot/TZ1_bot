...
server_url = config["telegram"]["server_log"]
bitrix_server_url = config["telegram"]["server_bitrix"]
...
async def send_data(update, context, action):
    ...
    data = {
        'action': action,
        'id': context.user_data.get('user_id')
    }
...
async def send_message(update, context, message_text):
    ...
    response = requests.post(bitrix_server_url, json=payload)
...
async def data_action(update, context):
    ...
    connection = mysql.connector.connect(
        host = config["sql"]["url_search"],  # IP-адрес виртуальной машины
        port = config["sql"]["url_search"],  # Порт базы данных
        user = config["sql"]["url_search"],  # Имя пользователя
        password = config["sql"]["url_search"],  # Пароль
        database = config["sql"]["url_search"]  # Имя базы данных
    )
    ...
    message = await update.callback_query.message.reply_text(
        ...
        f"\nТарифный план: {name} {speed} Мб./сек {cost} руб/мес \nБаланс: {float(data['data'][0]['deposit']):.2f}",
        reply_markup=reply_markup
    )
